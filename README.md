...
```
电源上电
 ├─ BIOS 模式:boot.asm(16 位实模式)→ stage2.asm(读 FAT12 定位 KERNEL.GCX)→
 │   pm32(32 位保护模式,加载 gcx 段)→ 建页表 → lm64(64 位长模式)→ 内核入口
 └─ UEFI 模式:固件直接加载 bootloader.efi → 读 gcx → 进入内核
```

- 内核以 **gcx 可执行格式** 分发(ELF 经 `gvtcier-gcx` 转换):含段表、入口、虚拟基址,引导层解析后加载
- BIOS 路径经 32 位保护模式兼容层过渡,最终与 UEFI 路径汇合于 64 位内核入口

### 地址空间与初始化顺序

- 内核映射于高半区 `0xFFFF800000000000+`(KERNEL_VIRT),用户态位于低地址
- 启动顺序:内存探测(e820)→ Buddy/堆/页表初始化 → GDT/IDT → APIC 定时器 → syscall 入口 → IPC 端点 → 驱动(AHCI/网络/音频)→ 裂变进程与任务创建 → Shell
- 每步自带 selftest(页表/分页/内存统计),启动日志可全程串口观察

### 子系统分层

| 层 | 组成 | 机制 |
|----|------|------|
| 硬件 | x86-64 CPU、AHCI/ATA、RTL8139、AC97、PS/2 | 直接寄存器/端口访问 |
| 内核 | 内存(Buddy/堆/页表/VMM)、任务调度、进程裂变、IPC 端点、权能 | 见各 docs |
| 驱动 | GvFAT、FAT、键盘、鼠标 | **独立用户态程序**,经消息与内核通信 |
| 用户 | GvShell、Demo、GvtcierC 程序 | 经 1-40 号系统调用访问内核服务 |

### 驱动模型与系统调用

- 驱动不内置于内核地址空间,而是作为独立任务运行,通过 IPC 端点消息与内核交互(如键盘经端点 1、鼠标经端点 3)
- 内核对外暴露 1-40 号系统调用(文件、图形、内存、进程、驱动、时间),`abi` 层提供封装
- 权能(Capability)控制资源访问:每任务持有权能表,按权限位(SEND/RECV)校验消息收发

### 数据流示例：文件读写

```
用户程序 ──syscall 30/31/37──▶ 内核 File 服务 ──AHCI 驱动──▶ 磁盘
  ▲                              │
  └──────────── 返回数据 ◀───────┘
```

- 内核维护 FAT 兼容层(GvFAT 的壳),GvFAT 驱动管理目录树/位图/元数据
- 读写路径经 AHCI 48 位 LBA(128PB 寻址),位图按块分配
